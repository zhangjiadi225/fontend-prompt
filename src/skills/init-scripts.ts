...
// 搜索脚本内容 - 用于在本地数据中搜索规范信息
export const SKILL_SCRIPT_CONTENT = `const fs = require('fs');
...
export async function generateSkillScripts(skillDir: string) {
  const scriptsDir = path.join(skillDir, "scripts");
  await fs.mkdir(scriptsDir, { recursive: true });

  const scriptPath = path.join(scriptsDir, "index.js");
  await fs.writeFile(scriptPath, SKILL_SCRIPT_CONTENT, "utf-8");
